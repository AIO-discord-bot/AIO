...
module.exports = {
    ...
    redeemKey: async (guild,userID, key) => {
        try {
            // Get key
            ...
            givePremium = async () => {
                ...
                settings.premium.details.premiumExpiresAt += keyData.premiumExpiresAt;
	...
	checkPremium: async (client) => {
		client.guilds.cache.forEach(async g => {
			const settings = await getSettings(g);
			const timeNow = new Date().getTime();
			if (settings.premium.enabled && timeNow > settings.premium.details.premiumExpiresAt) {
				console.log(`Guild ${g.name} (${g.id}) lost premium!!`);
				settings.premium.enabled = false;
				settings.save();
			};
		});
	}
