...
module.exports = class HelpCommand extends Command {
  ...
  async messageRun(message, args, invoke, prefix) {
    ...
    const viewhelp = new MessageEmbed()
      .setTitle("You are already viewing the help menu!")
      .setImage("https://media.discordapp.net/attachments/998404091157946480/1054012762709303386/x.png")


    // !help
    if (!trigger) {
      if (cache[`${message.guildId}|${message.author.id}`]) {
        return message.reply({ embeds: [viewhelp] });
      }
...
async function getHelpMenu({ client, guild }) {
  ...
  const embed = new MessageEmbed()
    ...
    .setDescription(
      ...
      `My prefix is \`!\` **or** \`/\` to use slash commands!\n\n **Help:** Support Server: discord.gg/verifiedbot [Click Here](https://discord.gg/verifiedbot)\n`+
...
function getSlashCategoryEmbeds(client, category) {
  ...
  arrSplitted.forEach((item, index) => {
    const embed = new MessageEmbed()
      ...
      .setFooter({text: `[${index + 1}/${arrSplitted.length}]`});
